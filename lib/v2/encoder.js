...
const debug = require('../debug')('hessian:v2:encoder');
const importLazy = require('import-lazy')(require);
const Long = importLazy('long');
const util = require('util');
const EncoderV1 = require('../v1/encoder');
const { isSafeNumberString } = require('../utils');
...
const INT_DIRECT_MIN = -0x10; // -16
const INT_DIRECT_MAX = 0x2f; // 47
const INT_ZERO = 0x90; // 144

const INT_BYTE_MIN = -0x800; // -2048
const INT_BYTE_MAX = 0x7ff; // 2047
const INT_BYTE_ZERO = 0xc8; // 200

const INT_SHORT_MIN = -0x40000; // -262144
const INT_SHORT_MAX = 0x3ffff; // 262143
const INT_SHORT_ZERO = 0xd4; // 212
...
proto.writeInt = function(val) {
  ...
  if (INT_DIRECT_MIN <= val && val <= INT_DIRECT_MAX) {
    ...
    const b0 = val & 0xff;
    const code = (val >> 8) + INT_BYTE_ZERO;
    this.byteBuffer.put(code).put(b0);
  } else if (INT_SHORT_MIN <= val && val <= INT_SHORT_MAX) {
    const b1b0 = val & 0xffff;
    const code = (val >> 16) + INT_SHORT_ZERO;
...
proto.writeLong = function(val) {
  ...
  if (typeof val === 'number') {
    if (val >= -8 && val <= 15) {
      ...
      const b0 = val & 0xff;
      const code = (val >> 8) + 0xf8;
      this.byteBuffer.put(code).put(b0);
    } else if (val >= -262144 && val <= 262143) {
      const b1b0 = val & 0xffff;
      const code = (val >> 16) + 0x3c;
      this.byteBuffer.put(code).putUInt16(b1b0);
...
proto.writeDouble = function(val) {
  ...
  if (intValue === val) {
    if (val === 0) {
      ...
      return this;
    } else if (intValue >= -0x80 && intValue < 0x80) {
      this.byteBuffer.put(0x69).put(intValue);
      return this;
    } else if (intValue >= -0x8000 && intValue < 0x8000) {
      this.byteBuffer.put(0x6a).putInt16(intValue);
      return this;
    } else if (intValue >= -0x80000000 && intValue < 0x80000000) {
...
proto.writeString = function(str) {
  ...
  while (length > 0x8000) {
    ...
    const tail = str.charCodeAt(strOffset + sublen - 1);
...
proto.writeType = function(type) {
  ...
  const ref = this._typeRefs.indexOf(type);
  if (ref >= 0) {
    this.writeInt(ref);
  } else {
    ...
    if (_typecache[type]) {
      ...
      const end = this.byteBuffer.position();
...
proto._writeObjectBegin = function(type, fields) {
  ...
  const ref = this._classRefs.indexOf(type);
  if (ref >= 0) {
    // o(x6f) ref
...
proto._writeObject = function(obj) {
  ...
  let keys = Object.keys(realObj);
  const ref = this._writeObjectBegin(className, keys);
  if (ref === -1) {
    // writeDefinition20
