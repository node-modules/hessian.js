...
proto.readObject = function (withType) {
  ...
  if (code === 0x4f) {
    ...
    classname = this._readUTF8String(size);
    this.classes.push(classname);
  } else if (code === 0x6f) {
    var ref = this.readInt();
    classname = this.classes[ref];
  } else {
    this.throwError('readObject', code);
  }

  debug('readObject %s, code: %s', classname, code);
  ...
  debug('readObject got fields: %j', fields);

  // skip o and ref
  if (debug.enabled) {
    debug('readObject skip code: 0x%s, ref: %d',
      this.byteBuffer.get().toString(16), this.readInt());
  } else {
