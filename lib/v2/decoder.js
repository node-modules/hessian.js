...
const debug = require('../debug')('hessian:v2:decoder');
const DecoderV1 = require('../v1/decoder');
const utils = require('../utils');
const isJavaException = require('../object').isJavaException;
const JavaExceptionError = require('../object').JavaExceptionError;
const decodeFnCtx = { JavaExceptionError };
...
proto.readInt = function() {
  const code = this.byteBuffer.get();
  // Compact int
  ...
  if (code >= 0xd0 && code <= 0xd7) {
    // Integers between -262144 and 262143 can be encoded in three bytes with the leading byte in the range xd0 to xd7.
    // value = ((code - 0xd4) << 16) + (b1 << 8) + b0;
    const b1 = this.byteBuffer.get();
    const b0 = this.byteBuffer.get();
...
proto._readObjectDefinition = function() {
  ...
  const classname = this._readUTF8String(size);
  const fieldsLength = this.readInt();
  // compose cache key with class name and fields length
  // more safely
  const cachekey = classname + '##' + fieldsLength;

  // get class definition from cache
  const cacheClz = this.classCache && this.classCache.get(cachekey);
  if (cacheClz) {
  ...
  if (this.classCache) {
    if (this.classCache.enableCompile) {
      let errorPropCount = 0;
      const gen = codegen([ 'decoder', 'withType' ], 'decode');
      gen('// %s', cachekey);
      gen('var result = {');
...
proto._readVariableLengthItems = function(list, withType) {
  let code = this.byteBuffer.get(this.byteBuffer.position());
  // Z
  while (code !== 0x5a) {
