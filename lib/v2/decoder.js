 ...
 * @see http://grepcode.com/file/repo1.maven.org/maven2/com.caucho/hessian/3.1.3/com/caucho/hessian/io/Hessian2Input.java#884
 * ```
 * value = (code - 0xe0) # LONG_ZERO = 0xe0
 * ```
 *
 * two octet longs:
...
proto.readString = function () {
  ...
  if (code >= 0x00 && code <= 0x1f) {
    ...
    // [x30-x33] b0 <utf8-data> # string of length 0-1023
    debug('read length 0-1023 strings');
    var len = (code - 0x30) * 256 + this.byteBuffer.get();
    str += this._readUTF8String(len);
  } else {
 ...
 * @api public
 */
proto.readClass = function (withType) {
  var code = this.byteBuffer.get();
  // assert(code, 0x4f);
  // class-def  ::= 'C(x43)' string int string*
  ...
  debug('readClass got %s fields: %j', classname, fields);
