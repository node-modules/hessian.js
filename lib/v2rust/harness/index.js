...
class ShapeCache {
    ...
    buildAnonymousObject(fields, fieldLen, utils) {
        ...
        if (fieldLen !== length) {
            throw new Error("对象key的hash校验失败, 请检查是否带有分隔符 ,");
        }
        return ret;
    }
    buildShape(fields, fieldLen, utils) {
        ...
        if (fieldsArray.length !== fieldLen) {
            throw new Error("对象key的hash校验失败, 请检查是否有带有分隔符 ,");
        }
        if (fieldsArray.some(x => x.includes('"'))) {
            throw new Error("key的名称不能带有引号，因为这个key会实时生成代码，防止代码注入");
        }
        const code = `
    ...
    withMapCache(cacheIdx, fields, fieldLen, utils) {
        ...
        if (cacheIdx !== null) {
            if (this.mapShapeCache[cacheIdx]) {
                ...
                throw new Error("cache must have value");
            }
        }
        if (fields === null || fieldLen === null) {
            throw new Error("At least one of fields and cacheIdx");
        }
...
const start = (shapeCache) => {
    ...
    function readChunk() {
        ...
        if (type === 1) {
            ...
            throw new Error("error chunk type");
        }
    }
    function read() {
        switch (outputAt(cursor++)) {
            ...
            default:
                throw new Error(`unkonw type at ${cursor}`);
        }
    }
...
function build(config) {
    ...
    if (typeof maxMapCacheLength !== "number") {
        throw new Error("maxCacheLength must be number");
    }
    if (maxMapCacheLength > maxU32) {
        throw new Error("最大长度为32位无符号整形最大值");
    }
    // rust 输出的buffer
    ...
    const { setState, setCache, reset } = start(shapeCache);
    return {
        preset: (input, offset = 0) => {
            ...
            if (offset > maxU32) {
                throw new Error("最大offset为32位无符号整形最大值");
            }
            if (input.length > maxU32) {
                throw new Error("input最大长度为32位无符号整形最大值");
            }
            if (input.length - offset > (outputBuffer.byteLength * 3) / 5) {
                throw new Error(`input太长了，达到了outputBufferLength: ${outputBuffer.byteLength}的3/5，可能会产生意想不到的结果，出于安全考虑抛出此异常，请精简返回结果，或加大输出buffer的size`);
            }
