...
const debug = require('../debug')('hessian:v1:encoder');
const utils = require('../utils');
const javaObject = require('../object');
const converts = require('../convert');
const is = require('../is_type_of');
...
proto._checkRef = function(obj) {
  const refIndex = this.objects.indexOf(obj);
  if (refIndex >= 0) {
...
proto.writeObject = function(obj) {
  ...
  const convertor = converts[obj.$class];
...
proto.writeArray = function(arr) {
  ...
  const isSimpleArray = Array.isArray(arr);
  let className = ''; // empty string meaning: `javaObject.DEFAULT_CLASSNAME.list`
  let realArray = arr;
  if (!isSimpleArray) {
    if (is.object(arr) && arr.$ == null) {
      return this.writeNull();
    }
    const isComplexArray = is.object(arr) &&
      is.string(arr.$class) && Array.isArray(arr.$);
    if (!isComplexArray) {
  ...
  const hasEnd = this._writeListBegin(realArray.length, className);
...
proto.write = function(val) {
  ...
  if (val == null || Number.isNaN(val) || typeof val === 'function' || utils.isRegExp(val)) {
    return this.writeNull();
  }
  switch (type) {
    case 'string':
    ...
    case 'number':
      ...
      if (val >= utils.MAX_INT_32 || val < -utils.MAX_INT_32) {
  ...
  const method = utils.getSerializer(val.$class);
  ...
  if (utils.isJavaObject(val.$class)) {
    if (utils.isDate(val.$) || !is.object(val.$)) {
