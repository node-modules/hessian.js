...
var BYTE_CODES = {};
var EMPTY_BYTES = new Buffer(0);

function Decoder(buf) {
  this.byteBuffer = ByteBuffer.wrap(buf || EMPTY_BYTES);
  this.refMap = {};
  this.refId = 0;
...
proto.readDate = function (withType) {
  ...
  var date = this.byteBuffer.getLong().toNumber();
...
proto._readUTF8String = function (len) {
  ...
  while (len--) {
    ...
    if (ch < 0x80) {
      ...
      var ch1 = this.byteBuffer.get();
      v = ((ch & 0x1f) << 6) + (ch1 & 0x3f);
    } else if ((ch & 0xf0) === 0xe0) {
      var ch1 = this.byteBuffer.get();
      var ch2 = this.byteBuffer.get();
