...
proto._checkLabel = function (method, label) {
  ...
  assert(labelIsOk, 'hessian %s get unexpect label: %s', method, label);
  return l;
};
...
proto.readDouble = function () {
  ...
  return utils.handleLong(this.byteBuffer.getDouble());
};
...
proto.readBytes = function () {
  ...
  while (label === 'b') {
    length = this.byteBuffer.readUInt16();
...
proto.readString = function () {
  ...
  while (label === 's') {
    slength = this.byteBuffer.readUInt16();
    startPos = this.byteBuffer.position();
    while (slength--) {
      head = this.byteBuffer.read();
      l = utils.lengthOfUTF8(head);
      this.byteBuffer.skip(l - 1);
    }
    str = this.getRawString(startPos, this.byteBuffer.position() - startPos);
    strs.push(str);
  }
