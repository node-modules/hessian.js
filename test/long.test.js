...
describe('long.test.js', function() {
  const longBuffer = Buffer.from([ 'L'.charCodeAt(0), 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x2c ]);

  it('should write {$class: "java.lang.Long", $: null} => null', function() {
    const obj = hessian.decode(hessian.encode({
  ...
  it('should write long 0', function() {
    assert.deepEqual(
      ...
      Buffer.from([ 'L'.charCodeAt(0), 0, 0, 0, 0, 0, 0, 0, 0 ])
    );
  });
  ...
  describe('v2.0', function() {
    it('should read compact long', function() {
      assert(hessian.decode(Buffer.from([ 0xe0 ]), '2.0') === 0);
      assert(hessian.decode(Buffer.from([ 0xd8 ]), '2.0') === -8);
      assert(hessian.decode(Buffer.from([ 0xef ]), '2.0') === 15);

      assert(hessian.decode(Buffer.from([ 0xf8, 0x00 ]), '2.0') === 0);
      assert(hessian.decode(Buffer.from([ 0xf0, 0x00 ]), '2.0') === -2048);
      assert(hessian.decode(Buffer.from([ 0xf7, 0x00 ]), '2.0') === -256);
      assert(hessian.decode(Buffer.from([ 0xff, 0xff ]), '2.0') === 2047);

      assert(hessian.decode(Buffer.from([ 0x3c, 0x00, 0x00 ]), '2.0') === 0);
      assert(hessian.decode(Buffer.from([ 0x38, 0x00, 0x00 ]), '2.0') === -262144);
      assert(hessian.decode(Buffer.from([ 0x3f, 0xff, 0xff ]), '2.0') === 262143);

      // four octet longs
      assert(hessian.decode(Buffer.from([ 0x77, 0x00, 0x00, 0x00, 0x00 ]), '2.0') === 0);
      assert(hessian.decode(Buffer.from([ 0x77, 0x00, 0x00, 0x01, 0x2c ]), '2.0') === 300);
      assert(
        hessian.decode(Buffer.from([ 0x77, 0x7f, 0xff, 0xff, 0xff ]), '2.0') === 2147483647
      );
      assert(
        hessian.decode(Buffer.from([ 0x77, 0x80, 0x00, 0x00, 0x00 ]), '2.0') === -2147483648
      );
    });

    it('should read normal long', function() {
      assert(
        hessian.decode(Buffer.from([ 0x4c, 0x00, 0x00, 0x00, 0x00, 0x80, 0x00, 0x00, 0x00 ]), '2.0') === 2147483648
      );
    });

    it('should write compact long', function() {
      // -8 ~ 15
      let buf = hessian.encode(java.long(0), '2.0');
      assert(buf.length === 1);
      assert(buf[0] === 0xe0);
      assert.deepEqual(buf, Buffer.from([ 0xe0 ]));

      buf = hessian.encode(java.long(-8), '2.0');
      assert(buf.length === 1);
      assert(buf[0] === 0xd8);
      assert.deepEqual(buf, Buffer.from([ 0xd8 ]));

      buf = hessian.encode(java.long(-7), '2.0');
      assert(buf.length === 1);
      assert(buf[0] === 0xd9);
      assert.deepEqual(buf, Buffer.from([ 0xd9 ]));

      buf = hessian.encode(java.long(15), '2.0');
      assert(buf.length === 1);
      assert(buf[0] === 0xef);
      assert.deepEqual(buf, Buffer.from([ 0xef ]));

      buf = hessian.encode(java.long(14), '2.0');
      assert(buf.length === 1);
      assert(buf[0] === 0xee);
      assert.deepEqual(buf, Buffer.from([ 0xee ]));
      ...
      assert(buf[0] === 0xf7);
      assert(buf[1] === 0xf7);
      assert.deepEqual(buf, Buffer.from([ 0xf7, 0xf7 ]));

      buf = hessian.encode(java.long(16), '2.0');
      assert(buf.length === 2);
      assert(buf[0] === 0xf8);
      assert(buf[1] === 0x10);
      assert.deepEqual(buf, Buffer.from([ 0xf8, 0x10 ]));

      buf = hessian.encode(java.long(255), '2.0');
      assert(buf.length === 2);
      assert(buf[0] === 0xf8);
      assert(buf[1] === 0xff);
      assert.deepEqual(buf, Buffer.from([ 0xf8, 0xff ]));

      buf = hessian.encode(java.long(-2048), '2.0');
      assert(buf.length === 2);
      assert(buf[0] === 0xf0);
      assert(buf[1] === 0);
      assert.deepEqual(buf, Buffer.from([ 0xf0, 0x00 ]));

      buf = hessian.encode(java.long(2047), '2.0');
      assert(buf.length === 2);
      assert(buf[0] === 0xff);
      assert(buf[1] === 0xff);
      assert.deepEqual(buf, Buffer.from([ 0xff, 0xff ]));
      ...
      assert(buf[1] === 0xff);
      assert(buf[2] === 0xff);
      assert.deepEqual(buf, Buffer.from([ 0x3f, 0xff, 0xff ]));
      ...
      assert(buf[1] === 0);
      assert(buf[2] === 0);
      assert.deepEqual(buf, Buffer.from([ 0x38, 0x00, 0x00 ]));
      ...
      assert(buf[1] === 0x08);
      assert(buf[2] === 0x00);
      assert.deepEqual(buf, Buffer.from([ 0x3c, 0x08, 0x00 ]));
      ...
      assert(buf[1] === 0xf7);
      assert(buf[2] === 0xff);
      assert.deepEqual(buf, Buffer.from([ 0x3b, 0xf7, 0xff ]));
      ...
      assert(buf[3] === 0x00);
      assert(buf[4] === 0x00);
      assert.deepEqual(buf, Buffer.from([ 0x77, 0x80, 0x00, 0x00, 0x00 ]));
      ...
      assert(buf[3] === 0xff);
      assert(buf[4] === 0xff);
      assert.deepEqual(buf, Buffer.from([ 0x77, 0x7f, 0xff, 0xff, 0xff ]));

      // L
      buf = hessian.encode(java.long(2147483648), '2.0');
      assert(buf.length === 9);
      assert.deepEqual(buf, Buffer.from([ 0x4c, 0x00, 0x00, 0x00, 0x00, 0x80, 0x00, 0x00, 0x00 ]));
    });
    ...
    it('should write { $class: "long": $: "..." } ok', function() {
      ...
      assert.equal(buf.length, 9);
      assert.deepEqual(buf, Buffer.from([ 0x4c, 0x00, 0x20, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 ]));
    });
  ...
  it('should decode with type', function() {
    assert.deepEqual(hessian.decode(Buffer.from([ 0x38, 0x00, 0x00 ]), '2.0', true), {
      $class: 'long',
      $: -262144,
    });
