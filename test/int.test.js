...
describe('int.test.js', function() {
  it('should read integer 300', function() {
    assert(
      hessian.decode(Buffer.from([ 'I'.charCodeAt(0), 0x00, 0x00, 0x01, 0x2c ])) === 300
    );
  });
  ...
  it('should write integer 300', function() {
    assert.deepEqual(
      hessian.encode(300),
      Buffer.from([ 'I'.charCodeAt(0), 0x00, 0x00, 0x01, 0x2c ])
    );
  });

  it('should write integer 0', function() {
    assert.deepEqual(hessian.encode(0), Buffer.from([ 'I'.charCodeAt(0), 0, 0, 0, 0 ]));
  });
  ...
  describe('v2.0', function() {
    it('should read compact integers', function() {
      assert(hessian.decode(Buffer.from([ 0x90 ]), '2.0') === 0);
      assert(hessian.decode(Buffer.from([ 0x80 ]), '2.0') === -16);
      assert(hessian.decode(Buffer.from([ 0xbf ]), '2.0') === 47);

      assert(hessian.decode(Buffer.from([ 0xc8, 0x00 ]), '2.0') === 0);
      assert(hessian.decode(Buffer.from([ 0xc0, 0x00 ]), '2.0') === -2048);
      assert(hessian.decode(Buffer.from([ 0xc7, 0x00 ]), '2.0') === -256);
      assert(hessian.decode(Buffer.from([ 0xcf, 0xff ]), '2.0') === 2047);

      assert(hessian.decode(Buffer.from([ 0xd4, 0x00, 0x00 ]), '2.0') === 0);
      assert(hessian.decode(Buffer.from([ 0xd0, 0x00, 0x00 ]), '2.0') === -262144);
      assert(hessian.decode(Buffer.from([ 0xd7, 0xff, 0xff ]), '2.0') === 262143);

      assert(
        hessian.decode(Buffer.from([ 'I'.charCodeAt(0), 0x00, 0x00, 0x00, 0x00 ]), '2.0') === 0
      );
      assert(
        hessian.decode(Buffer.from([ 'I'.charCodeAt(0), 0x00, 0x00, 0x01, 0x2c ]), '2.0') === 300
      );
    });
    ...
    it('should write compact integers', function() {
      // -16 ~ 47
      let buf = hessian.encode(0, '2.0');
      assert(buf.length === 1);
      assert(buf[0] === 0x90);
      assert.deepEqual(buf, Buffer.from([ 0x90 ]));

      buf = hessian.encode(1, '2.0');
      assert(buf.length === 1);
      assert(buf[0] === 0x91);
      assert.deepEqual(buf, Buffer.from([ 0x91 ]));

      buf = hessian.encode(-16, '2.0');
      assert(buf.length === 1);
      assert(buf[0] === 0x80);
      assert.deepEqual(buf, Buffer.from([ 0x80 ]));

      buf = hessian.encode(46, '2.0');
      assert(buf.length === 1);
      assert(buf[0] === 0xbe);
      assert.deepEqual(buf, Buffer.from([ 0xbe ]));

      buf = hessian.encode(47, '2.0');
      assert(buf.length === 1);
      assert(buf[0] === 0xbf);
      assert.deepEqual(buf, Buffer.from([ 0xbf ]));
      ...
      assert(buf[0] === 0xc0);
      assert(buf[1] === 0x00);
      assert.deepEqual(buf, Buffer.from([ 0xc0, 0x00 ]));

      buf = hessian.encode(-256, '2.0');
      assert(buf.length === 2);
      assert(buf[0] === 0xc7);
      assert(buf[1] === 0x00);
      assert.deepEqual(buf, Buffer.from([ 0xc7, 0x00 ]));
      ...
      assert(buf[0] === 0xc9);
      assert(buf[1] === 0x00);
      assert.deepEqual(buf, Buffer.from([ 0xc9, 0x00 ]));

      buf = hessian.encode(2047, '2.0');
      assert(buf.length === 2);
      assert(buf[0] === 0xcf);
      assert(buf[1] === 0xff);
      assert.deepEqual(buf, Buffer.from([ 0xcf, 0xff ]));
      ...
      assert(buf[1] === 0x00);
      assert(buf[2] === 0x00);
      assert.deepEqual(buf, Buffer.from([ 0xd0, 0x00, 0x00 ]));
      ...
      assert(buf[1] === 0xff);
      assert(buf[2] === 0xff);
      assert.deepEqual(buf, Buffer.from([ 0xd7, 0xff, 0xff ]));
      ...
      assert(buf[3] === 0x00);
      assert(buf[4] === 0x00);
      assert.deepEqual(buf, Buffer.from([ 'I'.charCodeAt(0), 0x00, 0x04, 0x00, 0x00 ]));
      ...
      assert(buf[3] === 0xff);
      assert(buf[4] === 0xff);
      assert.deepEqual(buf, Buffer.from([ 'I'.charCodeAt(0), 0xff, 0xfb, 0xff, 0xff ]));
    });
  });
});
